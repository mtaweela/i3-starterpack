...
this repo is fork of the repo https://github.com/addy-dclxvi/i3-starterpack.git

I have added some stuff related to ubuntu and some for my own configurations.
...
### Baclight Configuration

you can refer to this [link](https://itsfoss.com/fix-brightness-ubuntu-1310/) for more information.

run `ls /sys/class/backlight/`, you will find `intel_backlight` and in some cases other stuff.

then, open a terminal create the following configuration file if it does not exist:

```bash
sudo touch /usr/share/X11/xorg.conf.d/20-intel.conf
```

then edit the file `sudo gedit /usr/share/X11/xorg.conf.d/20-intel.conf` and add to it the following configuration:
...
finally, logout and log in again and you are fine now.

### SSH

this solution was found here in the following link:
https://yashagarwal.in/posts/2017/12/

this configures ssh agent to run in the begiging of the session and use the previous lunched agent if one is found.
...
now create the script with the following inside it:
...
### Edit Boot Splash Screen

this follow the answer on the question here:
https://askubuntu.com/questions/1046370/how-to-change-boot-splash-screen-in-18-04

go to /usr/share/plymouth/themes
...
You could find other things to be done for this to work.

You can check them in `.config/i3/config` file in the commented lines. (those will be transfered here later).